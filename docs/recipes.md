...
- Can we re-use changeset as a proposed change set? It's been saved to DB, but if it can be shown in
  a good "diff" view user can decide whether to apply the changeset to a specific source
- We need a way to describe commands to a source from a changeset, and a common API for
  SourcePlugins
...
3. Create a changeset with the proposed edits
4. (at a later time) apply the changeset to source(s) to acutally move the files
...
2. Run a processor on each
   1. It reads the file data accessor OR path and runs e.g. imagemagick
      1. That is saved to filesystem as a side-effect (e.g. regardless what source the asset is
         from)
      2. Or, that is saved, associated with a changeset, and can be applied to the source as a
         separate step?
...
3. User needs click a button in the top bar to complete the changeset (but technically nothing
   really bad happens if it's not closed, or we can just formally block new changesets if the last
   is open).
...
3. Save changeset back to source, e.g. removing these files
...
minimal work, and save that stat to a changeset, and then render it using a custom component from
the analyzer.
...
1. Start with a query or collection
2. Run an analyzer
3. Save the CSV as a side-effect (local file) or in the changeset payload
