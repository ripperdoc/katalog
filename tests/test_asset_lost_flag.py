...
async def test_clear_lost_flag_when_previous_true(db: None):
    ...
    change_set = MetadataChangeSet(
        loaded=loaded, staged=[Metadata(metadata_key_id=lost_key_id, value_type=METADATA_REGISTRY[ASSET_LOST].value_type, removed=False, provider_id=provider.id)]
    )
    change_set.staged[0].set_value(None)
    changed = await change_set.persist(asset=asset, snapshot=snap2)

    # One new row (removed=True) should be written
    all_rows = await Metadata.filter(asset=asset, metadata_key_id=lost_key_id).order_by("id")
...
async def test_clear_lost_noop_when_not_set(db: None):
    ...
    change_set.staged[0].set_value(None)
    changed = await change_set.persist(asset=asset, snapshot=snap1)
